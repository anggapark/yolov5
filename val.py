...
def run(
    ...
    LOGGER.info(pf % ("all", seen, nt.sum(), mp, mr, map50, map55, map60, map65, map70, map75, map))
    if nt.sum() == 0:
    ...
    if save_json and len(jdict):
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            eval.accumulate()
            eval.summarize()
            map, map50, map55, map60, map65, map70, map75 = eval.stats[:7]  # update results (mAP@0.5:0.95, mAP@0.5)
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return (mp, mr, map50, map55, map60, map65, map70, map75, map, *(loss.cpu() / len(dataloader)).tolist()), maps, t
